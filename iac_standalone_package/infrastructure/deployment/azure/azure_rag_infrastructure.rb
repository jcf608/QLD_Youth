...
class AzureRagInfrastructure < BaseInfrastructure
  ...
  def deploy
    ...
    create_database          # PostgreSQL database
    create_storage
  ...
  def load_configuration
    ...
    @config = {
      ...
      # PostgreSQL Database: Must be lowercase (Azure requirement)
      postgres_server: ENV['AZURE_POSTGRES_SERVER'] || generate_postgres_server_name(environment, default_location),
      postgres_database: ENV['AZURE_POSTGRES_DATABASE'] || 'utsv2',
      postgres_admin: ENV['AZURE_POSTGRES_ADMIN'] || 'utsv2admin',
      postgres_password: ENV['AZURE_POSTGRES_PASSWORD'] || generate_secure_password,
  ...
  def create_database
    section_header('PostgreSQL Database Creation')

    # Check if PostgreSQL server exists
    check_cmd = "az postgres flexible-server show --name #{@config[:postgres_server]} " \
                "--resource-group #{@config[:resource_group]} 2>&1"
    `#{check_cmd}`

    if $?.success?
      puts "ℹ️  PostgreSQL server '#{@config[:postgres_server]}' already exists"
      create_database_firewall_rules
      return
    end

    puts "Creating Azure Database for PostgreSQL Flexible Server '#{@config[:postgres_server]}'..."
    puts '⏳ This may take 3-5 minutes...'
    ...
    begin
      result = execute_command(
        ...
        "--admin-user #{@config[:postgres_admin]} " \
        "--admin-password '#{@config[:postgres_password]}' " \
        ...
        "--output json",
        description: 'Creating PostgreSQL Flexible Server'
      )
      ...
      record_resource(
        ...
        id: server_data['id'],
        endpoint: server_data['fullyQualifiedDomainName']
      )

      puts "✅ PostgreSQL server '#{@config[:postgres_server]}' created"
      ...
      if e.message.include?('already exists') || e.message.include?('ServerNameNotAvailable')
        puts
        puts "⚠️  WARNING: PostgreSQL server name '#{@config[:postgres_server]}' is already in use."
        puts "   Generating a new unique name..."
        puts

        # Generate new name and retry
        @config[:postgres_server] = generate_postgres_server_name
        puts "   Trying with new name: #{@config[:postgres_server]}"
        puts
        retry
      else
        raise
  ...
  def create_application_database
    puts
    puts "Creating application database '#{@config[:postgres_database]}'..."

    check_cmd = "az postgres flexible-server db show " \
                "--server-name #{@config[:postgres_server]} " \
                "--resource-group #{@config[:resource_group]} " \
                "--database-name #{@config[:postgres_database]} 2>&1"
    `#{check_cmd}`

    if $?.success?
      puts "ℹ️  Database '#{@config[:postgres_database]}' already exists"
      return
    end
    ...
    execute_command(
      ...
      "--server-name #{@config[:postgres_server]} " \
      "--resource-group #{@config[:resource_group]} " \
      "--database-name #{@config[:postgres_database]} " \
      "--output json",
    ...
    record_resource(
      ...
      id: "#{@config[:postgres_server]}/#{@config[:postgres_database]}"
    )

    puts "✅ Database '#{@config[:postgres_database]}' created"
  end

  def create_database_firewall_rules
    puts
    puts 'Configuring database firewall rules...'

    # Allow Azure services to access the database
    puts "  • Adding Azure services access rule..."
    azure_rule_result = `az postgres flexible-server firewall-rule show \
      --resource-group #{@config[:resource_group]} \
      --name #{@config[:postgres_server]} \
    ...
    unless $?.success?
      execute_command(
        ...
        "--output json",
        description: 'Creating Azure services firewall rule'
      )
      puts "     ✅ Azure services access enabled"
    else
      puts "     ℹ️  Azure services rule already exists"
    end

    # For development: Allow all IPs (remove in production!)
    puts "  • Adding public access rule (development only)..."
    dev_rule_result = `az postgres flexible-server firewall-rule show \
      --resource-group #{@config[:resource_group]} \
      --name #{@config[:postgres_server]} \
      --rule-name AllowAll 2>&1`
    ...
    unless $?.success?
      execute_command(
        ...
        "--output json",
        description: 'Creating public access firewall rule'
      )
      puts "     ✅ Public access enabled (DEVELOPMENT ONLY)"
      puts "     ⚠️  WARNING: Remove this rule in production!"
    else
      puts "     ℹ️  Public access rule already exists"
    end
  end
  ...
  def configure_security
    ...
    postgres_host = "#{@config[:postgres_server]}.postgres.database.azure.com"
    puts "DATABASE_URL='postgresql://#{@config[:postgres_admin]}:#{@config[:postgres_password]}@#{postgres_host}:5432/#{@config[:postgres_database]}?sslmode=require'"
    puts "DB_HOST='#{postgres_host}'"
    puts "DB_PORT='5432'"
    puts "DB_USERNAME='#{@config[:postgres_admin]}'"
    puts "DB_PASSWORD='#{@config[:postgres_password]}'"
    puts "DB_NAME='#{@config[:postgres_database]}'"
    puts
    puts '# Azure Storage'
    ...
    begin
      ...
      output = {
        ...
        deployment_config: {
          ...
          postgres_admin: @config[:postgres_admin],
  ...
  def check_resource_status
    ...
    # Check PostgreSQL
    postgres_result = `az postgres flexible-server show --name #{@config[:postgres_server]} --resource-group #{@config[:resource_group]} 2>&1`
    puts postgres_result.include?('could not be found') ? '❌ PostgreSQL Server: Not Found' : '✅ PostgreSQL Server: Exists'
  ...
  def generate_postgres_server_name(environment = 'dev', location = 'centralus')
    # Azure PostgreSQL server names are globally unique
    # Must be 3-63 chars, lowercase, numbers, hyphens (can't start/end with hyphen)
    # Format: uts-{env}-pg-{region_code}-{random}
    # Note: Must be lowercase - Azure requirement
    env_code = environment.to_s.downcase
    region_code = get_region_code(location)
    "uts-#{env_code}-pg-#{region_code}-#{SecureRandom.hex(2)}"
  end

  def generate_secure_password
    # Generate a secure password for PostgreSQL
    # Azure requires: 8-128 chars, mix of uppercase, lowercase, numbers, and symbols
    chars = [
      ('a'..'z').to_a,
      ('A'..'Z').to_a,
      ('0'..'9').to_a,
    ...
    password += (0...12).map { chars.sample }
